...
def plot_iteration(
    ...
    # --- place circles ---
    # Derive font size from the rendered sub-cell size in display points so
    # text stays inside the circle regardless of grid dimensions.
    sub_pts   = (fig_w * fig.dpi) / total_cols   # one sub-cell in display pts
    circle_radius = 0.42                          # in data units (sub-cells)
    font_size     = 12.0
    circle_color  = "#3a86ff"
    text_color    = "white"
    ...
    for row in rows:
        ...
        val_str = (f"{row.value:.0f}"
                   if row.value == float(row.value) and abs(row.value) < 1e9
                   else f"{row.value:.3g}")
...
def generate_plots(rows: list[Row], output_dir: Path) -> None:
    ...
    for it in iterations:
        plot_iteration(it, by_iteration[it], max_x, max_y, output_dir)
        print(f"  Saved: iteration_{it:04d}.png")
