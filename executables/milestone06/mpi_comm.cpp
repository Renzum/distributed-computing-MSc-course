...
MPICommunicator::MPICommunicator(int domain_width, int domain_height) {
    ...
    MPI_Comm_rank(cart, &mpi_rank);
    MPI_Cart_coords(cart, mpi_rank, 2, coords);

    std::cout << fmt::format("{:d}: Dimensions: [{:d}, {:d}]", mpi_rank,
                             dims[0], dims[1])
              << std::endl;
    std::cout << fmt::format("{:d}: Coordinates: [{:d}, {:d}]", mpi_rank,
                             coords[0], coords[1])
              << std::endl;

    // The four neighbors; with non-periodict boundaries every rank has all
    // four
    ...
    std::cout << fmt::format("{:d}: My up is {:d}", mpi_rank, up) << std::endl;
    std::cout << fmt::format("{:d}: My down is {:d}", mpi_rank, down)
              << std::endl;
    std::cout << fmt::format("{:d}: My left is {:d}", mpi_rank, left)
              << std::endl;
    std::cout << fmt::format("{:d}: My right is {:d}", mpi_rank, right)
              << std::endl;
    ...
    lattice_width =
        domain_width / dims[1] + ghost_layers.left + ghost_layers.right;

    lattice_height =
        domain_height / dims[0] + ghost_layers.down + ghost_layers.up;
    ...
    mpi_implementation = MPIImplementationFactory::get_implementation();
}
...
//     if (left != MPI_PROC_NULL) {
//         auto left_subview =
//             Kokkos::subview(distribution_function, 1, Kokkos::ALL,
//             Kokkos::ALL);
...
//             distribution_function, lattice_width - 1, Kokkos::ALL,
//             Kokkos::ALL);
...
//     if (right != MPI_PROC_NULL) {
//         auto right_subview = Kokkos::subview(
//             distribution_function, lattice_width - 2, Kokkos::ALL,
//             Kokkos::ALL);
...
//     if (left != MPI_PROC_NULL) {
//         auto left_subview =
//             Kokkos::subview(distribution_function, 0, Kokkos::ALL,
//             Kokkos::ALL);
...
//     if (down != MPI_PROC_NULL) {
//         auto down_subview =
//             Kokkos::subview(distribution_function, Kokkos::ALL, 1,
//             Kokkos::ALL);
//         Kokkos::deep_copy(up_down_send_gpu_buffer, down_subview);
//     }

//     MPI_Sendrecv(up_down_send_gpu_buffer.data(), up_down_buffer_len,
//     MPI_DOUBLE,
//                  down, 0, up_down_recv_gpu_buffer.data(), up_down_buffer_len,
//                  MPI_DOUBLE, up, 0, cart, &status);

//     if (up != MPI_PROC_NULL) {
//         auto up_subview = Kokkos::subview(distribution_function, Kokkos
//         ::ALL,
...
//     if (down != MPI_PROC_NULL) {
//         auto down_subview = Kokkos::subview(distribution_function, Kokkos
//         ::ALL,
//                                             0, Kokkos::ALL);
