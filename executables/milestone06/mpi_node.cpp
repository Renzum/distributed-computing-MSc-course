...
Node::Node(int global_domain_size) {
    ...
    left_right_gpu_buffer = GPUBuffer("Horzintal GPU Buffer", lattice_height);
...
void Node::communicate_up(
    ...
    MPI_Sendrecv(up_down_send_buffer.data(), up_down_buffer_len, MPI_DOUBLE, up,
                 0, up_down_recv_buffer.data(), up_down_buffer_len, MPI_DOUBLE,
                 down, 0, cart, &status);
